-- Add CONVERTED_TO_DATASET to priority_action enum
ALTER TYPE priority_action ADD VALUE IF NOT EXISTS 'converted';

-- Update the conversion function to use the correct enum value
...
BEGIN
  ...
  -- 3. Create catalog_metadata record - ONLY copy name to title and slug
  -- Everything else is NULL/default and editable by producer
  ...
  VALUES (
    p_assignee_org_id,
    priority_rec.name,  -- ONLY bring name to title
    new_slug,           -- Generated slug (locked)
    TRUE,
    p_priority_dataset_id,
    p_user_id,
    p_user_id,
    'DRAFT',
    'PUBLIC'  -- Default, producer can change
  ) RETURNING id INTO new_dataset_id;

  -- 4. Log the conversion with correct enum value
  ...
  VALUES (
    p_priority_dataset_id,
    p_user_id,
    'converted',  -- Use valid enum value
